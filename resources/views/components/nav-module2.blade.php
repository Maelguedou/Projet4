<nav x-data="{ open: false }" class="  bg-gradient-to-r from-[#e4ece8] via-[#96f1b8] to-[#39b867] text-white shadow-md">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            <!-- Logo -->
            <div class="flex items-center gap-6">
                
                <a href="{{ route('home2') }}" class="flex items-center gap-3 group">
                    ...
                    <span class="hidden sm:inline-block font-bold text-white tracking-wide ">
                        Espace Public
                    </span>
                </a>
                ...
                <div class="hidden md:flex md:items-center md:space-x-2">
                    @if (auth()->user()->isAdmin())
                        <x-nav-link :href="route('dashboard')" :active="request()->routeIs('home')" class="px-3 py-2 text-sm text-white/90 hover:text-white">
                            {{ __('Dashboard') }}
                        </x-nav-link>

                    @else
                        <x-nav-link :href="route('enseignant.dashboard')" :active="request()->routeIs('home')" class="px-3 py-2 text-sm text-white/90 hover:text-white">
                            {{ __('Dashboard') }}
                        </x-nav-link>    
                    @endif
